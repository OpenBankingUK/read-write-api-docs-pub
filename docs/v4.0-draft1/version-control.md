...
| VRP | <ul><li>`InSession` and `OffSession` renamed to `UK.OB.InSession` & `UK.OB.OffSession`</li><li>`OBRisk2` object added without ExtendedPurpose field</li><li>`ContractPresentIndicator` typo fixed</li><li>Added guidance on dynamic reference information for VRP payments</li></ul> |
