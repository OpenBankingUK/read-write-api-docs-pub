...
| All | OBL enums changed from `UK.OBIEIE.*` to `UK.OBIE.*` |
...
| VRP | <ul><li>`InSession` and `OffSession` renamed to `InSession` & `OffSession`</li><li>`OBRisk2` object added without ExtendedPurpose field</li><li>`ContractPresentIndicator` typo fixed</li><li>Added guidance on dynamic reference information for VRP payments</li></ul> |
