...
| RelatedDate |0..1 |OBDomesticStandingOrder3/RemittanceInformation/Structured/ReferredDocumentInformation/RelatedDate |Date associated with the referred document line |ISODateTime | | |
...
| CreditorReferenceInformation |0..1 |OBDomesticStandingOrder3/RemittanceInformation/Structured/CreditorReferenceInformation|Reference information provided by the creditor to allow the identification of the underlying documents |OBCreditorReferenceInformation| | |
| Code |0..1 |OBDomesticStandingOrder3/RemittanceInformation/Structured/CreditorReferenceInformation/Code |Type of creditor reference, as published in an external creditor reference type code set. For a full description see `ExternalCreditorReferenceType1Code` [here](https://github.com/OpenBankingUK/External_Interal_CodeSets) |ExternalCreditorReferenceType1Code| | |
| Issuer |0..1 |OBDomesticStandingOrder3/RemittanceInformation/Structured/CreditorReferenceInformation/Issuer |Entity that assigns the credit reference type |Max35Text | | |
| Reference |0..1 |OBDomesticStandingOrder3/RemittanceInformation/Structured/CreditorReferenceInformation/Reference |EUnique reference, as assigned by the creditor, to unambiguously refer to the payment transaction.<br>Usage: If available, the initiating party should provide this reference in the structured remittance information, to enable reconciliation by the creditor upon receipt of the amount of money.<br> If the business context requires the use of a creditor reference or a payment remit identification, and only one identifier can be passed through the end-to-end chain, the creditor's reference or payment remittance identification should be quoted in the end-to-end transaction identification. |Max35Text | | |
| Invoicer |0..1 |OBDomesticStandingOrder3/RemittanceInformation/Structured/Invoicer | Identification of the organisation issuing the invoice, when it is different from the creditor or ultimate creditor. |OBInvoicer| | |
| Invoicee |0..1 |OBDomesticStandingOrder3/RemittanceInformation/Structured/Invoicee |Identification of the party to whom an invoice is issued, when it is different from the debtor or ultimate debtor |OBInvoicee| | |
| TaxRemittance |0..1 |OBDomesticStandingOrder3/RemittanceInformation/Structured/TaxRemittance |Provides remittance information about a payment made for tax-related purposes |Max140Text| | |
| AdditionalRemittanceInformation |0..3|OBDomesticStandingOrder3/RemittanceInformation/Structured/AdditionalRemittanceInformation | Additional information, in free text form, to complement the structured remittance information |OBAdditionalRemittanceInformation| | |
